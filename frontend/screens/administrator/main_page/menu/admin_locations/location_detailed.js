...
const LocationDetailed = ({ navigation, route }) => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const filteredLocation = Object.entries(location).reduce((acc, [key, value]) => {
                if (value !== null && key != "createdAt" && key != "updatedAt") { // Include în obiect doar proprietățile care nu sunt null
                    acc[key] = value;
                }
                return acc;
            }, {});
            ...
            await axios.put(`http://${IP}:8080/api/location/update-location`, {
                ...filteredLocation
            }, {
                headers: {
                    'Authorization': 'Bearer ' + accessToken
        ...
        <LinearGradient style={styles.linearGradient} colors={[COLORS.secondary, COLORS.primary]}>
            <KeyboardAwareScrollView>
                <View style={styles.titleContainer}>
                    ...
                    >
                        <View style={{ position: 'absolute',left: -100, top: 40}}>
                            
                                <AntDesign name="left" size={40} color={COLORS.pink} />
                        </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => navigation.navigate('DeleteLocation', location_id)}
                        hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}  // Mărește zona de atingere
                        style={{  padding: 30, zIndex: 10 }}  // Mărește zona de atingere fizic
                    >
                        <View style={{ position: 'absolute',right: -100, top: 40}}>
                            <MaterialIcons name="delete" size={40} color={COLORS.pink} />
                        </View>
                    </TouchableOpacity>
                    
                </View>
                <View style={{marginTop: 50}}>
                    <TextInput style={styles.titleText} value={location.name} onChangeText={(text) => handleChange('name', text)} multiline />
                    <TextInput style={styles.subtitleText} value={location.address} onChangeText={(text) => handleChange('address', text) } multiline/>
                    <Text style={styles.descriptionText}>Description:</Text>
                    <TextInput style={styles.descriptionText} value={location.description} onChangeText={(text) => handleChange('description', text)} multiline />
                </View>
                <MapView
                    style={styles.map}
                    initialRegion={{
