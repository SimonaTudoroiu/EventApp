...
const AdminEvents = ({ navigation }) => {
    ...
    const fetchLocations = async () => {
        ...
        try {
            const accessToken = await AsyncStorage.getItem('accessToken');
            const response = await axios.get(`http://${IP}:8080/api/event/find-locations`, {
                headers: {
                    'Authorization': 'Bearer ' + accessToken
                }
            });
            ...
            for (const location of locations) {
                try {
                    ...
                    locationsData[location.name] = sortEvents(response2.data.data);
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity onPress={() => navigation.navigate('AdminEventDetailed', item.event_id)}>
    ...
    return (
        <LinearGradient style={styles.linearGradient} colors={[COLORS.secondary, COLORS.primary]}>
            <View style={styles.titleContainer}>
                <TouchableOpacity onPress={() => navigation.navigate("AdminMenu")} style={[styles.iconTouch, styles.leftIcon]}>
                    <Ionicons name="menu" size={50} color={COLORS.pink} />
                </TouchableOpacity>
                <Text style={styles.titleText}>Events</Text>
                <TouchableOpacity onPress={() => navigation.navigate("AdminAccount")} style={[styles.iconTouch, styles.rightIcon]}>
            ...
            <View style={styles.bottomContainer}>
                ...
                {Object.keys(locations).map(location => (
                    <View key={location}>
                        <Text style={styles.title}>{capitalizeFirstLetter(location)}</Text>
                        <FlatList
                            horizontal
